...
#
df=pd.read_pickle('E:/research/wangying/version4/data_github/1-3_up_grid_based.pkl')
NS=4
max_lon_grid=2239
max_lat_grid=2134
folder_w='E:/research/wangying/version4/data_github/2-LMD/'
...
def extraction_LMD_all(df,max_lat_grid,max_lon_grid,NS,folder_w,n_jobs_set):
    ...
    def extraction_local_hotspots(df,NS,max_lat_grid,max_lon_grid):
        ...
        df.loc[[lat_p_grid,lon_p_grid],'whether_seed']=1
        df.loc[[lat_p_grid,lon_p_grid],'seed_id']=df_seed['seed_id'].values
            ...
            if seed_id_matrix[grid_classification[0],grid_classification[1]]!=0:
                ...
                if len(seed_id_arr)==1:
                    ...
                    if len(max_gravity_index)==1:
                        belong_seed_id=seed_id_arr[max_gravity_index]
                    else:
    ...
    #3-1 确定每个seed_id的shape，并将shape形式记录下来
    df_1=pd.read_pickle(folder_w+'1-df.pkl')
    ...
    def compute_shape(df_1,grid_classification_matrix_2):
        ...
        for seed_id in seed_id_array:
            ...
            if len(shape_index)!=0:
                seed_shape_id_record_list.append(shape_record_arr[shape_index,0])
            else:
        ...
        df_shape_record=df_shape_record=pd.DataFrame(shape_record_arr,columns=['shape_id','shape_size','shape_record'])
    ...
    def final_merge(df_3_1,df_shape_record_3_1,grid_classification_matrix_2,df_shape_threshold_3_3):
        ...
        #输出'lat_p_grid','lon_p_grid'，'seed_id'
        #其他字段后续再处理
